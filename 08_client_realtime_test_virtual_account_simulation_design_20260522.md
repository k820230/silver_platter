...
1. 기본 가상 계좌 초기 자산은 KRW 100,000,000, USD 0, 초기 보유 종목 없음으로 한다.
2. replay 데이터는 3년 일봉과 가능 시 6개월 분봉을 제공한다.
3. 체결 지연 기본값은 1초다.
4. 부분체결은 bar 거래량의 최대 5%까지 체결한다.
5. 호가 데이터가 없으면 지정가가 bar high/low를 관통한 경우 체결로 판단한다.
6. simulation 세금 화면은 기본 노출하되 신고용 flag는 false로 표시한다.
7. snapshot 보존 기간은 1년이다.
8. session archive는 3년 보존하고 사용자가 수동 삭제할 수 있다.
9. 전략 승격은 최소 20개 simulation/paper session 결과를 검토한 뒤 가능하다.
10. multi-session은 사용자당 동시 3개까지 허용한다.

## 20.1 결정 반영 사항

- 실시간 테스트 모드는 가상 계좌 초기 자산, 체결 지연, 부분체결 정책, replay 데이터 제공 범위를 기본 설정으로 가져야 한다.
- 위 기본 설정의 구체적인 숫자와 범위는 24번 결정 레지스터의 MVP 기본값으로 확정한다.
