...
1. 종목별 fine-tuning은 일봉 2년 이상, 핵심 feature 결측 5% 이하, base model 대비 검증 성능 5% 이상 개선 또는 동등 이상일 때 적용한다.
2. MVP 모델 family는 tree ensemble을 1순위, baseline linear model을 비교 기준으로 둔다.
3. 사용자 지정 horizon은 1거래일부터 1년까지 허용한다.
4. drift threshold는 PSI 0.25 초과 또는 최근 20개 예측 interval hit rate 50% 미만으로 둔다.
5. 모델 artifact는 `/home/jhkim5/silver_platter_models`에 저장하고 active와 최근 10개 또는 2년 중 긴 기간을 보존한다.
6. 예측 confidence는 calibration 40%, data quality 30%, 최근 예측오차 30%로 산출한다.
