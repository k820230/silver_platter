...
1. 초기 사업 그룹 taxonomy는 표준 산업분류 대분류와 내부 유사 사업 그룹을 병행하고, 반도체/2차전지/자동차/금융/플랫폼/클라우드/바이오/에너지/방산/소비재를 우선 seed로 둔다.
2. 그룹별 최대 비중은 warning 20%, block 30%다.
3. 그룹 VaR/CVaR는 historical scenario 방식으로 산출한다.
4. 다각화 기업의 매출 비중이 1년 이상 오래된 경우 동일가중 보정하고 degraded flag를 표시한다.
5. ETF 구성종목 데이터는 provider 가능 범위에서 월 1회 갱신한다.
6. headline relevance는 entity match 50%, business keyword 30%, source confidence 20%로 시작한다.
7. 수동 검토 queue는 `live_trader` 이상 권한, crisis 5분, danger 15분 SLA로 둔다.
...
- 그룹 유동성 한도는 그룹 내 당일 신규 주문금액 합계/그룹 구성 종목 20거래일 평균 거래대금 합계 최대 5%를 MVP 초기값으로 사용한다.
- 그룹별 최대 비중과 최대 손실 상세값은 24번 결정 레지스터의 MVP 기본값으로 확정한다.
