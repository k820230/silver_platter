...
class ProviderTests(TestCase):
    ...
    def test_kis_daily_price_provider_normalizes_domestic_chart_rows(self):
        class FakeTransport:
            def __init__(self):
                self.posts = []
                self.gets = []
        ...
        bars = list(provider.get_price_bars("005930.KS"))

        self.assertEqual("kis_domestic_daily_price", provider.metadata.provider_code)
        self.assertEqual(1, len(bars))
        self.assertEqual("005930", bars[0].security_id)
        self.assertEqual(datetime(2026, 5, 22, 16, 0, 0), bars[0].bar_ts)
        self.assertEqual(70500.0, bars[0].close_price)
        self.assertEqual(1234.0, bars[0].volume)
        self.assertEqual(87000000.0, bars[0].turnover_krw)
        self.assertEqual("/oauth2/tokenP", transport.posts[0][0])
        self.assertEqual(
            "/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice",
            transport.gets[0][0],
        )
        self.assertEqual("Bearer token-1", transport.gets[0][1]["authorization"])
        self.assertEqual("FHKST03010100", transport.gets[0][1]["tr_id"])
