...
For local secrets, create an untracked `.env`. The guarded smoke scripts load simple `KEY=VALUE` entries from `.env`, and Docker Compose reads the same file for variable substitution before applying the tracked `.env.example` defaults.
...
`./scripts/goldilocks_backup.sh` uses `BACKUP_BASE_DIR`, `GOLDILOCKS_BACKUP_POLICY`, and `GOLDILOCKS_BACKUP_COMMAND`; it runs the configured command inside a dated backup directory, writes a checksum-backed manifest only when backup files are produced, and skips without creating a fake manifest when no command is configured. `./scripts/goldilocks_logical_backup` provides the default app-level logical backup payload for local validation, writing Goldilocks migration status and table counts without copying secrets.
`./scripts/kis_orderable_smoke` is read-only and skips unless KIS query credentials and `KIS_API_BASE_URL` are configured.
`POST /api/securities/search` and new `POST /api/watchlist/items` securities prefetch KR domestic daily price history through KIS and store it in Goldilocks `SP.price_bar` when KIS query credentials and Goldilocks ODBC are configured. The default target is `HISTORY_PREFETCH_TARGET_BAR_COUNT=300` daily bars, with `HISTORY_PREFETCH_LOOKBACK_DAYS` used to choose the default date range. KIS access tokens are reused through `KIS_TOKEN_CACHE_PATH` or the default `RAW_DATA_DIR/kis_access_token.json` cache to avoid repeated OAuth issuance.
